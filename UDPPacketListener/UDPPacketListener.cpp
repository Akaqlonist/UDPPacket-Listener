// UDPPacketListener.cpp : Defines the entry point for the console application.
//
...
//padding
byte padding_value[8] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 };
...
unsigned char * hex2char(unsigned char * key, int len)
{
	unsigned char * key_str = new unsigned char[1];
	for (int i = 0; i < len / 2; i++)
	{
		unsigned char h1 = key[i * 2];
		unsigned char h0 = key[i * 2 + 1];
...
int main()
{
	...
	printf("Enter port number you would like to sniff : ");
	scanf_s("%d", &port_no);

	//printf("Enter base64 decoded key : ");

	//////////// create blow fish engine //////
	unsigned char key[] = "6161616161616161";
	int length = strlen((char*)key);
	unsigned char * key_str = hex2char(key, length);
	blowFish = new CBlowFish(key_str, length / 2);

	//blowfish test
	/*
	unsigned char test_hex_data[] = "6161616161616161";
										
	unsigned char * test_char_data = hex2char(test_hex_data, 16);

	unsigned char * out = new unsigned char[16];
	blowFish->Encrypt(test_char_data, out, 8);
...
void PrintUdpPacket(char *Buffer, int Size)
{
	...
	if (ntohs(udpheader->dest_port) == port_no) //check port
	{

		//discard less than 110byte packets
		if (Size < 110)
		{
			return;
		}

		//get data size of current packet
		int originSize = Size - sizeof(UDP_HDR) - iphdr->ip_header_len * 4;

		int i;

		//get starting pointer of data part in packet
		char * data = Buffer + iphdrlen + sizeof(UDP_HDR);

		//
		std::vector<unsigned char> vector_data = std::vector<unsigned char>();

		//trim first 2 byte & store data in char vector
		for (i = 2; i < originSize; i++)
			vector_data.push_back((unsigned char)data[i]);

		//get trimmed size
		int size = originSize - 2;

		//calculate unencrypted part size
		int unencryptedSize = size % 8;

		//number of bytes to be added at the end  ; padding
		int padSize = 8 - unencryptedSize;

		std::vector<unsigned char> padding = std::vector<unsigned char>();

		for (i = 0; i < unencryptedSize; i++)
		{
			padding.push_back(vector_data[size - unencryptedSize + i]);
		}
		for (i = 0; i < padSize; i++)
		{
			padding.push_back(padding_value[padSize - 1]);
		}

		unsigned char * in = new unsigned char[padding.size()];
		for (i = 0; i < padding.size(); i++)
			in[i] = padding[i];

		unsigned char * out = new unsigned char[padding.size() * 2];
	
		blowFish->Encrypt(in, out, padding.size());
		
		for (i = 0; i < unencryptedSize; i++)
		{
			vector_data[size - unencryptedSize + i] = out[i];
		}

		for (i = 0; i < padSize + 8; i++)
		{
			vector_data.push_back(out[i + unencryptedSize]);
		}

		delete in;
		delete out;
		in = new unsigned char[vector_data.size()];
		out = new unsigned char[vector_data.size() * 2];
		for (i = 0; i < vector_data.size(); i++)
			in[i] = vector_data[i];

		blowFish->Decrypt(in, out, vector_data.size());
		for (i = 0; i < vector_data.size(); i++)
		{
			printf_s("%c", out[i]);
		}
